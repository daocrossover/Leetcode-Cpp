...
public:
    vector<int> findRightInterval(vector<vector<int>>& intervals) {
        vector<int> res;
        map<int, int> hash; // map is ordered
        // the starts are unique so record the index of each interval
        for (int i = 0; i < intervals.size(); ++i) {
            hash[intervals[i][0]] = i;
        }
        for (auto i: intervals) {
            // binary search to find lower bound of current end
            auto iter = hash.lower_bound(i[1]);
            if (iter == hash.end()) {
                res.push_back(-1);
            } else {
